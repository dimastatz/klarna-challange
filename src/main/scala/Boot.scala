...
object Boot {
  def main(args: Array[String]): Unit = {
    val inputFile = Try(args(0)).getOrElse("/temp/input.txt")
    val alphabetFile = Try(args(2)).getOrElse("/temp/alphabet.txt")
    val pw = new PrintWriter(Try(args(1)).getOrElse("/temp/output.txt"))
  ...
  def parseLine(lines: Seq[String], template: Map[String, Int]): Array[Int] = {
    ...
    digits.map(i => Try(template(i)).getOrElse(-1))
  }
  ...
  def parseLine(lines: Seq[String], index: Int = 0): Array[String] = {
    if(index + 3 > lines.head.length) Array()
       ...
       Array(key.mkString(",").trim) ++ parseLine(lines, index + 3)
    }
  }
}
